...
class ScanNetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, id):
        ...
        """
        id = self.id_list[id]
        cam_pose = np.loadtxt(os.path.join(self.data_path, self.scene, "pose",  '{0}.txt'.format(id)), delimiter=' ')

        # Read depth image and camera pose
        # depth_im = cv2.imread(os.path.join(self.data_path, self.scene, "depth", 'frame-{0:06d}.depth.pgm'.format(id)), -1).astype(
        #     np.float32)
...
class ReplicaDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, n_imgs, scene, data_path, max_depth, id_list=None):
        ...
        self.extr = np.loadtxt(os.path.join(data_path, scene, 'traj.txt')).reshape(-1, 4, 4)
