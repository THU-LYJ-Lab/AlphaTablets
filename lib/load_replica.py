...
from recon.utils import load_sfm_pose, load_replica_pose
...
def load_replica_data(args, kf_list=[], image_skip=1, load_plane=True):
    imgs = []
    ...
    intr, sfm_poses, sfm_camprojs = load_replica_pose(args.input_dir)
    ...
    normal_names = sorted(glob.glob(os.path.join(normal_dir, "*")))
    ...
    all_depth_names = sorted(glob.glob(os.path.join(aligned_depth_dir, "*")))[start:end:image_skip]
    all_depths = []
    ...
    all_normal_names = sorted(glob.glob(os.path.join(normal_dir, "*")))[start:end:image_skip]
    all_normals = []
